...
//			--------------------------checklist 
			...
			function list_checklist(){
				...
				$.ajax({
					...
					txt = ``;
					for(var i = 0; i < res.length; i ++){
						txt += `<div class="check" data-chno="${res[i].checklist_no}">
						<h5 class="check_title" style="display:inline;">
						* ${res[i].check_title}
						</h5>
						<a href="#" id="delete_check" style="margin-left:10px; font-size : 15px;">
						삭제</a>`;
			...
			function listList(checklist_no){
				$.ajax({
					...
					$.each(res, function(i, item){
						...
						var txt =``;
						if(item.checked == 0){
							txt += `<li id="list_${res[i].list_no}" 
							data-lno="${item.list_no}" style="list-style :none;padding-left : 5px;">
							<input type="checkbox" name="list_info" value="${item.checked}">
							${item.list_info}
							<a href="#" id="delete_list" style="margin-left:10px; font-size : 15px;">삭제</a>
							</input>
							</li>`;
						}else{
							txt += `<li data-lno="${item.list_no}">
							
							<input type="checkbox" name="list_info" value="${item.checked}" checked>
							<a href="#" id="delete_list" style="margin-left:10px; font-size : 15px;">삭제</a>
							${item.list_info}</input></li>`;
						}
