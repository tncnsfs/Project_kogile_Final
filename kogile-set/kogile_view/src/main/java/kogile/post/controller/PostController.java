...
//	public PostVO insertPost(PostVO post){
	public PostVO insertPost(PostVO post){
		...
		System.out.println("post 행 81-포스트컨트롤: " + post);
	...
	public PostVO detailPost(@PathVariable("p_no") int p_no, Model model) {
		
		PostVO post = service.detailPost(p_no);

		System.out.println("detailPost");
		
		int postPno = post.getP_no();
		
		model.addAttribute("p_No", postPno);
		System.out.println("postPno== Post컨트롤에 있음: " + postPno);
		
		System.out.println("Post?: p_NO" + p_no);
